...
@testset "Particle Splitting with Resize" begin
    ...
    test_indices = [1, 2, 3]
    n_childs_exclude_center = TrixiParticles.nchilds(fluid_system,
                                                     fluid_system.particle_refinement.splitting_pattern) -
                              1
    n_expected_new_particles = n_childs_exclude_center * length(test_indices)

    particle_mass_max = refinement.spacing_ratio * fluid_system.cache.reference_mass[1]
    particle_mass_diff = particle_mass_max -
                         TrixiParticles.hydrodynamic_mass(fluid_system, 1)
    fluid_system.cache.reference_mass[test_indices] = fluid_system.cache.reference_mass[test_indices] .-
                                                      (particle_mass_diff + 0.1)

    # Manual splitting and resizing
    _n_add_particles = TrixiParticles.collect_split_candidates!(fluid_system, refinement, v,
                                                                u, semi)
    ...
    @testset "Test `apply_splitting!`" begin
        for (idx, particle) in enumerate(test_indices)
            expected_mass = old_masses[idx] / TrixiParticles.nchilds(fluid_system,
                                                   refinement.splitting_pattern)
            expected_h = old_h[idx] * refinement.splitting_pattern.alpha

            @test isapprox(TrixiParticles.hydrodynamic_mass(fluid_system, particle),
                           expected_mass)
            @test isapprox(TrixiParticles.smoothing_length(fluid_system, particle),
                           expected_h)

            for child_local in 1:n_childs_exclude_center
                child_global = (idx - 1) * n_childs_exclude_center + child_local

                @test isapprox(refinement.resize_buffer.masses_new[child_global],
                               expected_mass)
                @test isapprox(refinement.resize_buffer.smoothing_lengths_new[child_global],
                               expected_h)
                @test isapprox(refinement.resize_buffer.densities_new[child_global],
                               old_rho[idx])

                expected_child_vel = old_vel[idx]
                expected_child_pos = old_pos[idx] +
                                     old_h[idx] *
                                     refinement.splitting_pattern.relative_position[child_local]

                child_offset = (child_global - 1) * NDIMS
                for dim in 1:NDIMS
                    linear_idx = child_offset + dim
                    @test isapprox(refinement.resize_buffer.velocities_new[linear_idx],
                                   expected_child_vel[dim])
                    @test isapprox(refinement.resize_buffer.positions_new[linear_idx],
                                   expected_child_pos[dim])
                end
            end
        end
    end
